...
- **Desktop working memory core** with typed owner/session/conversation/project contexts, SQLite migrations, source events, facts, evidence, async embedding jobs, conservative post-turn capture, explicit remember/forget handling, canonical-key replacement inside scope, hybrid retrieval across active owner scopes, and compact memory prompt injection for future turns.
