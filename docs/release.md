...
- macOS `.dylib` and `.jnilib` files are excluded from the `desktopApp` jar and shipped as standalone app resources so Developer ID signing can timestamp them before notarization.
