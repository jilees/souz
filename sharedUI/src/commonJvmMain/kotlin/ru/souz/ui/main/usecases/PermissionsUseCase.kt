...
class PermissionsUseCase(
    ...
    suspend fun runOnboardingIfNeeded() {
        ...
        val displayText = if (permissionPromptService.isSandboxed) {
    ...
    fun registerNativeHook(): Boolean {
        if (permissionPromptService.isSandboxed) {
            l.info("Skipping global hotkey registration in sandboxed build")
            return false
        }
        if (permissionPromptService.isHeadless) {
            l.info("Skipping global hotkey registration in headless environment")
            return false
        }
        permissionPromptService.requestInputMonitoringAccessPromptIfNeeded()
        return permissionPromptService.registerNativeHook()
    }

    fun unregisterNativeHook() {
        permissionPromptService.unregisterNativeHook()
    }
        ...
        permissionWatcherJob = scope.launch {
            ...
            while (isActive) {
                if (canRegisterNativeHookNow()) {
                    ...
                    if (!permissionPromptService.relaunchApp()) {
    ...
    suspend fun rejectPendingPermissionRequest(requestId: Long?) {
    ...
    private fun canRegisterNativeHookNow(): Boolean {
        if (permissionPromptService.isHeadless) return false
        return permissionPromptService.canRegisterNativeHookNow()
    }
