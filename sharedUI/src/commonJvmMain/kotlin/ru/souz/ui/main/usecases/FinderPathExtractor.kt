...
class FinderPathExtractor(
    ...
    fun extract(content: String): List<FinderPathItem> =
        extractExistingPaths(content).map { path ->
            FinderPathItem(
                path = path,
                displayName = pathMetadataProvider.displayName(path),
                isDirectory = pathMetadataProvider.isDirectory(path)
            )
        }
    ...
    private fun resolveExistingPath(rawCandidate: String): String? {
        ...
        return attempts
            .asSequence()
            .mapNotNull { pathMetadataProvider.normalizePath(it) }
            .filter { it != "/" && pathMetadataProvider.exists(it) }
